...
public class DashboardEndpointsTests : IClassFixture<VokasiaApiFactory>
{
    ...
    private async Task<(HttpClient Client, Guid TeacherId)> AuthenticatedTeacherClientAsync(Guid tenantId)
    {
    ...
    public async Task GetSchoolDashboard_ComputesPercentagePendingApprovalsAndFlaggedList()
    {
        ...
        var (client, teacherId) = await AuthenticatedTeacherClientAsync(tenantId);
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var placementFilled = new Placement { Id = Guid.NewGuid(), TenantId = tenantId, StudentId = studentFilled.Id, CompanyId = company.Id, PeriodId = periodId, TeacherId = teacherId, Status = PlacementStatus.Active };
            ...
            var placementEmpty = new Placement { Id = Guid.NewGuid(), TenantId = tenantId, StudentId = studentEmpty.Id, CompanyId = company.Id, PeriodId = periodId, TeacherId = teacherId, Status = PlacementStatus.Active };
            ...
            // Placement milik guru lain: Teacher hanya boleh melihat placement yang ditugaskan
            // kepadanya, jadi slot ini tidak ikut persentase dashboard.
            var otherStudent = new Student { Id = Guid.NewGuid(), TenantId = tenantId, FullName = "Siswa Guru Lain", MajorId = Guid.NewGuid(), Classroom = "XII C" };
            ...
            var otherSlot = new JournalSlot { Id = Guid.NewGuid(), TenantId = tenantId, PlacementId = otherPlacement.Id, Date = Today, Status = JournalSlotStatus.Filled };
            ...
            db.Students.AddRange(studentFilled, studentEmpty, otherStudent);
            db.Placements.AddRange(placementFilled, placementEmpty, otherPlacement);
            db.JournalSlots.AddRange(slotFilled, slotEmpty, otherSlot, pendingSlot);
    ...
    {
        var tenantId = Guid.NewGuid();
        var (client, _) = await AuthenticatedTeacherClientAsync(tenantId);
    ...
    public async Task GetSchoolDashboard_FlaggedList_ExcludesGreenIncludesAmberWithCorrectReason()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var greenPlacement = new Placement { Id = Guid.NewGuid(), TenantId = tenantId, StudentId = greenStudent.Id, CompanyId = company.Id, PeriodId = periodId, TeacherId = teacherId, Status = PlacementStatus.Active };
            ...
            var amberPlacement = new Placement { Id = Guid.NewGuid(), TenantId = tenantId, StudentId = amberStudent.Id, CompanyId = company.Id, PeriodId = periodId, TeacherId = teacherId, Status = PlacementStatus.Active };
    ...
    public async Task GetSchoolDashboard_ScopesToRequestedPeriodOnly_OtherPeriodDataNotLeaked()
    {
        ...
        using (var scope = _factory.Services.CreateScope())
        {
            ...
            var otherPlacement = new Placement { Id = Guid.NewGuid(), TenantId = tenantId, StudentId = otherStudent.Id, CompanyId = company.Id, PeriodId = otherPeriod.Id, TeacherId = teacherId, Status = PlacementStatus.Active };
