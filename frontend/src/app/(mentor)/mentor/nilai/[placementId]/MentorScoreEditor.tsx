...
export function MentorScoreEditor({ placementId }: MentorScoreEditorProps) {
  ...
  const mentorAspects = assessment.aspects.filter((aspect) => isMentorSide(aspect.kind));
  ...
  const aspects: ScoreAspectInput[] = mentorAspects.map((aspect) => ({
    id: aspect.aspectId,
  ...
  const values = Object.fromEntries(mentorAspects.map((aspect) => [aspect.aspectId, aspect.mentorValue]));
  const comments = Object.fromEntries(mentorAspects.map((aspect) => [aspect.aspectId, aspect.mentorComment]));
    ...
    <div className="flex flex-col gap-4">
      ...
      <ScoreForm
        ...
        onSave={async (aspectId, value, comment) => {
          await apiClient.post(`/placements/${placementId}/assessment/mentor-scores`, [{ aspectId, value, comment }]);
        }}
      />
    </div>
  );
