...
> **Catatan keamanan (29 Jul 2026):** acceptance criteria lama yang memakai
> `X-Acting-Tenant` sudah digantikan. Runtime sekarang selalu mengabaikan header
> tersebut; pergantian tenant oleh SuperAdmin hanya boleh melalui alur impersonation
> eksplisit yang teraudit.
...
- `TenantResolutionMiddleware.Invoke(ctx, next)` — tujuan: isi `ITenantContext` dari claims (`tenant_id`,`sub`,`role`); override via header `X-Acting-Tenant` adalah requirement lama yang sudah dibatalkan.
...
- Given SuperAdmin mengirim `X-Acting-Tenant`, When akses data tenant, Then header diabaikan dan hasil tenant tetap kosong tanpa alur impersonation teraudit.
